    ...
    try {
      const response = await authFetch(`${import.meta.env.VITE_API_URL}/api/test/crear`, {
        ...
        body: JSON.stringify({
          ...
          pregunta: "",
          orden_pregunta: 1,
          preguntas: { [testForm.nombre_test]: 1 },
        }),
      });
        ...
        <div className="flex-1 space-y-6">
          <div className="rounded-2xl border bg-white p-6 shadow-sm">
            <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
              <div>
                <h1 className="text-2xl font-bold text-slate-900">Panel de Administrador</h1>
                <p className="text-sm text-slate-500">Gestiona tests, suscripciones y empresas desde un solo lugar.</p>
              </div>
              <div className="flex items-center gap-2 rounded-full bg-blue-50 px-3 py-2 text-sm text-blue-700">
                <ShieldCheck className="h-4 w-4" />
                Sesión activa para {session.nombre}
          ...
          <div className="grid gap-4 md:grid-cols-3">
            <Card>
              <CardContent className="pt-6 text-center">
                <div className="text-2xl font-bold text-blue-600">{summary.tests}</div>
                <div className="text-sm text-slate-500">Tests disponibles</div>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="pt-6 text-center">
          ...
          {activeSection === "tests" ? (
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2"><FlaskConical className="h-5 w-5" /> Crear test</CardTitle>
                <CardDescription>Registra una nueva evaluación para el sistema.</CardDescription>
              </CardHeader>
              <CardContent>
                <form onSubmit={createTest} className="space-y-4">
                  <div className="grid gap-4 md:grid-cols-2">
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="nombre_test">Nombre del test</Label>
                      <Input id="nombre_test" value={testForm.nombre_test} onChange={(e) => setTestForm({ ...testForm, nombre_test: e.target.value })} placeholder="Ej. PHQ-9" />
                    </div>
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="descripcion_test">Descripción</Label>
                    <Textarea id="descripcion_test" value={testForm.descripcion} onChange={(e) => setTestForm({ ...testForm, descripcion: e.target.value })} placeholder="Describe el propósito del test" />
                  </div>
                  <Button type="submit" disabled={submitting}>{submitting ? "Guardando..." : "Crear test"}</Button>
                </form>
                ...
                <div className="mt-6 space-y-2">
                  ...
                  {tests.length === 0 ? <p className="text-sm text-slate-500">Aún no hay tests creados.</p> : (
                    <div className="space-y-2">
                      {tests.map((test) => (
                        <div key={test.id_test} className="rounded-xl border p-3">
                          <p className="font-medium">{test.nombre_test}</p>
                          <p className="text-sm text-slate-500">{test.descripcion}</p>
                        </div>
                      ))}
                    </div>
